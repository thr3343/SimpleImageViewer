...
{
    explicit ComputePipeline(MemSys2 const& Memsys) noexcept : MemSys2(Memsys){};
    
    VkDescriptorSetLayout compDescriptorSetLayout=setupDescriptorSetLayout();
    vmaBuffer compSSBO=allocBuff(width*height*4,VK_BUFFER_USAGE_STORAGE_BUFFER_BIT|VK_BUFFER_USAGE_TRANSFER_DST_BIT,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
